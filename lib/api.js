...
export const authApi = {
  ...
  kakaoLogin: (kakaoId, email, nickname) =>
      api.post('/auth/kakao', { kakao_id: kakaoId, email, nickname }),
};
...
export const calendarApi = {
  ...
  getAuthorizeUrl: (provider, redirectUri, state) =>
    api.get(`/calendar/connections/${provider}/authorize`, {
      params: { redirect_uri: redirectUri, state },
    }),

  exchangeOAuthCode: ({ provider, code, redirectUri, state }) =>
  ...
  setDefault: (provider) =>
    api.patch('/calendar/connections/default', { provider }),

  disconnect: (provider) =>
    api.delete(`/calendar/connections/${provider}`),

  createEventForCall: (callId, payload = {}) =>
    api.post(`/calls/${callId}/calendar-events`, payload),
};
